...
import { Car, Check, Clock, Route, Trash2, UserRound, X } from "lucide-react";
...
function CompanyBookingsDetailMetric({
  ...
  return (
    <div className="min-w-0 rounded-[18px] border border-[var(--taxis-workspace-border)] bg-white/82 px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.88)]">
      ...
      <p className="mt-1 truncate font-semibold text-[13px] text-zinc-950">
        {primary}
      </p>
      {secondary ? (
    ...
    >
      ...
      >
        ...
        <div className="taxis-overlay-header relative z-10 flex items-start justify-between gap-4 border-zinc-100/70 border-b px-6 pt-6 pb-5">
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2 flex min-w-0 items-center gap-2.5">
              <span
                className="taxis-data-id block min-w-0 truncate font-semibold text-[22px] text-zinc-950 tracking-tight"
                id={titleId}
                title={bookingId}
              >
                {bookingId}
          ...
          <button
            ...
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[var(--taxis-workspace-border)] bg-white/88 text-[var(--taxis-workspace-text-muted)] shadow-sm transition-all hover:border-[var(--taxis-workspace-border-strong)] hover:text-zinc-950 active:scale-[0.99]"
            onClick={onClose}
            type="button"
          >
            <X size={20} />
        ...
        <div className="taxis-workspace-scrollbar flex-1 space-y-6 overflow-y-auto px-6 pt-6 pb-7">
          ...
          <section>
            ...
            <div className="space-y-2">
              {passengers.map((passenger) => (
                <div
                  className="ride-detail-passenger-row taxis-workspace-ease-standard flex items-center gap-4 rounded-[20px] border border-[var(--taxis-workspace-border)] bg-white/88 p-4 shadow-[0_14px_38px_-34px_rgba(15,23,42,0.45),inset_0_1px_0_rgba(255,255,255,0.9)]"
                ...
                >
                  <CompanyDashboardAvatar
                    ...
                    name={passenger}
                  />
                  <p className="min-w-0 flex-1 truncate font-semibold text-[15px] text-zinc-950">
                    {passenger}
                  </p>
                </div>
              ))}
          ...
          <section className="ride-detail-info-card space-y-5 rounded-[22px] border border-[var(--taxis-workspace-border)] bg-white/88 p-5 shadow-[0_16px_42px_-36px_rgba(15,23,42,0.48),inset_0_1px_0_rgba(255,255,255,0.92)]">
            ...
            <div className="flex items-center gap-2">
              <Clock
                ...
                size={16}
              />
              <p className="font-semibold text-[15px] text-zinc-950">
                {row.pickupDateLabel}, {row.pickupTimeLabel}
              </p>
            </div>
          </section>

          <section className="rounded-[22px] border border-[var(--taxis-workspace-border)] bg-white/88 p-5 shadow-[0_16px_42px_-36px_rgba(15,23,42,0.48),inset_0_1px_0_rgba(255,255,255,0.92)]">
          ...
          <section className="rounded-[22px] border border-[var(--taxis-workspace-border)] bg-white/88 p-5 shadow-[0_16px_42px_-36px_rgba(15,23,42,0.48),inset_0_1px_0_rgba(255,255,255,0.92)]">
            ...
            <div className="space-y-3">
              {vehicleUnits.map((unit) => {
                ...
                return (
                  <div
                    className="rounded-[20px] border border-[var(--taxis-workspace-border)] bg-white/84 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.9)]"
                    key={unit.unitId}
                  >
                    <div className="mb-4 flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="font-semibold text-[14px] text-zinc-950">
                          {unit.label}
                        </p>
                        {unit.publicId ? (
                    ...
                    {shouldShowAssignment ? (
                      <div className="flex items-start gap-4">
                        ...
                        <div className="min-w-0 flex-1 space-y-2">
                          {unit.driverName ? (
                            <p className="flex min-w-0 items-center gap-2 font-semibold text-[14px] text-zinc-950">
                              <UserRound
              ...
              <div className="space-y-2">
                {history.map((item) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-semibold text-[13px] text-zinc-950">
                        {item.label}
                      </p>
        ...
        <div className="relative z-10 border-zinc-100/80 border-t bg-white/84 px-6 py-5">
          <button
          ...
          {cancelError ? (
            <p className="mt-3 text-center font-medium text-[12px] text-red-700">
              {cancelError}
            </p>
...
function CompanyBookingsEmptyState({ copy }: { copy: CompanyBookingsCopy }) {
  return (
    <div className="taxios-bookings-empty rounded-[22px] px-7 py-12 text-center">
      <p className="font-semibold text-[15px] text-zinc-900 tracking-tight">
        {copy.emptyTitle}
      </p>
      <p className="mx-auto mt-2 max-w-md font-medium text-[13px] text-[var(--taxis-workspace-text-muted)] leading-relaxed">
          ...
          {rows.map((row) => (
            ...
            >
              <td className="taxis-workspace-table-cell taxios-bookings-cell rounded-l-[16px] border-y border-l px-4 py-3.5">
                <span className="taxis-data-value block font-semibold text-[17px] text-zinc-950 leading-none tracking-tight">
                  {row.pickupTimeLabel}
                </span>
              ...
              <td className="taxis-workspace-table-cell taxios-bookings-cell rounded-r-[16px] border-y border-r px-4 py-3.5 text-right">
                ...
                <button
                  ...
                  className="sr-only focus:not-sr-only focus:absolute focus:top-1/2 focus:right-4 focus:z-10 focus:-translate-y-1/2 focus:rounded-xl focus:border focus:border-[var(--taxis-workspace-surface-rim)] focus:bg-white focus:px-3 focus:py-2 focus:font-semibold focus:text-[12px] focus:text-[var(--taxis-workspace-text-secondary)] focus:shadow-[0_10px_24px_-18px_rgba(15,23,42,0.28)]"
                  onClick={(event) => {
